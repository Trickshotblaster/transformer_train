...
Original file is located at
    https://colab.research.google.com/github/Trickshotblaster/nn-practices/blob/main/TransformerV4.ipynb
"""
...
class MultiHeadAttention(nn.Module):
  def __init__(self, d_model, n_heads, n_heads_kv=None):
    super().__init__()
    self.d_model = d_model
    self.n_heads = n_heads
    self.n_heads_kv = n_heads_kv if n_heads_kv else n_heads
    assert d_model % n_heads == 0 and d_model % n_heads_kv == 0, "d_model must be divisible by n_heads"
    assert n_heads % n_heads_kv == 0, "n_heads must be divisible by kv heads"
    self.n_head_groups = n_heads // n_heads_kv
    self.d_key = self.d_model // self.n_heads

    kv_emb_dim = d_model // n_heads * self.n_heads_kv

    self.wq = nn.Linear(d_model, d_model)
    self.wk = nn.Linear(d_model, kv_emb_dim)
    self.wv = nn.Linear(d_model, kv_emb_dim)

    self.wo = nn.Linear(d_model, d_model)
  def forward(self, ins, mask=None):
    batch_size, seq_len, d_model = ins.size()
    Q = self.wq(ins).view(batch_size, seq_len, self.n_heads_kv, self.n_head_groups, self.d_key)
    K = self.wk(ins).view(batch_size, seq_len, self.n_heads_kv, 1, self.d_key)
    V = self.wv(ins).view(batch_size, seq_len, self.n_heads_kv, 1, self.d_key)

    if self.n_heads != self.n_heads_kv:
      K = K.expand(batch_size, seq_len, self.n_heads_kv, self.n_head_groups, self.d_key)
      V = V.expand(batch_size, seq_len, self.n_heads_kv, self.n_head_groups, self.d_key)
...
class DecoderBlock(nn.Module):
  def __init__(self, vocab_size, d_model, n_heads, n_heads_kv=None, ff_dim=None, dropout=0.1):
    super().__init__()
    self.d_model = d_model
    self.n_heads = n_heads
    self.n_heads_kv = n_heads_kv
    self.dropout = nn.Dropout(dropout)
    self.MHA = MultiHeadAttention(d_model, n_heads, n_heads_kv)
    if not ff_dim:
      self.ff_dim = 4*d_model
    else:
      self.ff_dim = ff_dim
    self.MLP = MLP(d_model, self.ff_dim, d_model)
...
class GPT(nn.Module):
  def __init__(self, vocab_size, block_size, n_layers=2, n_heads=4, n_heads_kv=None, d_model=64, ff_dim=None, dropout=0.1):
    super().__init__()
    ...
    self.decoder_stack = nn.ModuleList([
        DecoderBlock(vocab_size, d_model, n_heads, n_heads_kv, ff_dim=ff_dim, dropout=dropout) for _ in range(n_layers)
    ])
    self.final_ln = nn.LayerNorm(d_model)
    self.output_proj = MuReadout(d_model, vocab_size, bias=False)
...
base_model = GPT(enc.n_vocab, block_size, n_layers, n_heads, n_heads_kv, d_model, ff_dim=ff_dim, dropout=0.1)
delta_model = GPT(enc.n_vocab, block_size, n_layers, n_heads, n_heads_kv, 448, ff_dim=4096, dropout=0.1)
my_GPT = GPT(enc.n_vocab, block_size, n_layers, n_heads, n_heads_kv, d_model, ff_dim=ff_dim, dropout=0.1) #enc.n_vocab

set_base_shapes(my_GPT, base_model, delta=delta_model)
...
optim = mup.optim.AdamW(my_GPT.parameters(), lr=max_lr, fused=True)
...
import wandb
wandb.init(project="mike", config={
   "learning_rate": max_lr,
   "dataset_pretrain": "OpenOrca",
   "dataset_finetune": None,
   "max_steps": max_steps,
   "max_finetune_steps": 10000,
...
for step in range(max_steps + 1):
  ...
  if step % log_interval == 0:
    ...
    print(f"step {step:5d} | loss: {loss_accum.item():.6f} | norm: {norm:.4f} | dt: {step_time*1000:.2f}ms | tok/s: {(batch_size * block_size * grad_accum_steps) / step_time}")
  if step % val_interval == 0:
    with torch.no_grad():
      ...
      if val_loss < best_val_loss:
        ...
        torch.save(my_GPT.state_dict(), 'best_model_pretrain.pth')
      torch.save(my_GPT.state_dict(), 'latest_model_pretrain.pth')
...
for step in range(max_steps + 1):
  ...
  if step % val_interval == 0:
    ...
    with torch.no_grad():
      ...
      if val_loss < best_val_loss:
        best_val_loss = val_loss
        torch.save(my_GPT.state_dict(), 'best_model_finetune.pth')
      torch.save(my_GPT.state_dict(), 'latest_model_finetune.pth')
      print(f"Val loss for step {step}: {val_loss}")
