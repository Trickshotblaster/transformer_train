...
class MLP(nn.Module):
  def __init__(self, in_size, hidden_size, out_size):
    super().__init__()
    self.l1 = BitLinear(in_size, hidden_size)
    self.l2 = BitLinear(hidden_size, out_size)
    self.gelu = nn.GELU()
...
my_GPT = BitNetTransformer(num_tokens=enc.n_vocab, dim=d_model, depth=n_layers, heads=n_heads, ff_mult=4) #GPT
...
for step in range(max_steps + 1):
  ...
  for micro_step in range(grad_accum_steps):
    ...
    with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
      logits = my_GPT(x)
      loss = F.cross_entropy(logits.view(-1, enc.n_vocab), y.view(-1))
  ...
  if step % val_interval == 0:
    with torch.no_grad():
      ...
      for val_step in range(val_loss_steps):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
          logits = my_GPT(val_x)
          loss = F.cross_entropy(logits.view(-1, enc.n_vocab), val_y.view(-1))
        val_loss += loss
      val_loss /= val_loss_steps
      wandb.log({"val-loss": val_loss.item()}, commit=True)
      if val_loss < best_val_loss:
        best_val_loss = val_loss
        torch.save(my_GPT.state_dict(), 'best_model_pretrain_bit.pth')
      torch.save(my_GPT.state_dict(), 'latest_model_pretrain_bit.pth')
      print(f"Val loss for step {step}: {val_loss}")
      my_GPT.train()
my_GPT.load_state_dict(torch.load('latest_model_pretrain_bit.pth'))
my_GPT.eval()
...
for step in range(max_steps + 1):
  ...
  if step % val_interval == 0:
    ...
    with torch.no_grad():
      ...
      if val_loss < best_val_loss:
        best_val_loss = val_loss
        torch.save(my_GPT.state_dict(), 'best_model_finetune_bit.pth')
      torch.save(my_GPT.state_dict(), 'latest_model_finetune_bit.pth')
      print(f"Val loss for step {step}: {val_loss}")
my_GPT.load_state_dict(torch.load('latest_model_finetune_bit.pth'))
my_GPT.eval()
